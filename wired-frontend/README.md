...
| **3. Repository** | Integrate with **application state**: TanStack Query (`useQuery` / `useMutation`), cache keys, invalidation; call adapters and shared infrastructure (`queryClient`). No raw UI. | `use*Mutation.ts`, `use*Query.ts`, `useDashboard.ts`, `useDocument.ts`, etc. |
| **4. Adapter** | Talk to the **network** (and similar I/O): build requests, map responses; may include API-facing rules. Depends on shared HTTP/socket helpers, not on screens. | `*Adapter.ts` (e.g. `loginAdapter`, `dashboardAdapter`, `documentAdapter`) |
...
```mermaid
flowchart TB
  subgraph feature [Feature folder e.g. login or dashboard]
