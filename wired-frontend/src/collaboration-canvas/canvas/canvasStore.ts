...
export const useCanvasStore = create<CanvasStore>((set) => ({
  ...
  replaceShapes: (shapes) =>
    set((state) => {
      ...
      return {
        selectedId: state.selectedId && presentIds.has(state.selectedId) ? state.selectedId : null,
        editingId: state.editingId && presentIds.has(state.editingId) ? state.editingId : null,
        history: {
          past: [],
          present: shapes,
          future: [],
  ...
  setStickyPreset: (fill, textColor) => set({ stickyColor: fill, stickyTextColor: textColor }),
  undo: () =>
    set((state) => {
      ...
      return {
        selectedId: state.selectedId && prevIds.has(state.selectedId) ? state.selectedId : null,
        editingId: state.editingId && prevIds.has(state.editingId) ? state.editingId : null,
        history: {
  ...
  redo: () =>
    set((state) => {
      ...
      return {
        selectedId: state.selectedId && nextIds.has(state.selectedId) ? state.selectedId : null,
        editingId: state.editingId && nextIds.has(state.editingId) ? state.editingId : null,
        history: {
          past: [...state.history.past, state.history.present],
          present: next,
