...
function ColorSwatch({ label, value, onChange }: { label: string; value: string; onChange: (hex: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <div className="relative h-7 w-9 shrink-0 overflow-hidden rounded border border-slate-200 dark:border-slate-600">
        ...
        <input
          type="color"
          aria-label={transparent ? `${label} — no fill, pick a color` : `${label} color`}
          value={hex}
          title={transparent ? 'No fill — click to choose a color' : 'Fill color'}
...
function OpacitySlider({
  ...
  return (
    <label className={`flex min-w-[7rem] items-center gap-2 ${disabled ? 'opacity-40' : ''}`}>
      <span className="whitespace-nowrap text-[10px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">
        {label}
      </span>
      <input
...
function StrokeWidthSlider({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  const v = Math.min(STROKE_WIDTH_MAX, Math.max(STROKE_WIDTH_MIN, Number.isFinite(value) ? value : 0));
  return (
...
export function CanvasSelectionStyleBar({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      let idealCx: number;
      let idealTop: number;
      if (s.kind === 'sticky' || s.kind === 'triangle' || s.kind === 'kite') {
      ...
      if (prev && Math.abs(prev.cx - next.cx) < 0.35 && Math.abs(prev.top - next.top) < 0.35) {
        rafId = requestAnimationFrame(tick);
        return;
      }
  ...
  return (
    ...
    >
      <div className="pointer-events-auto flex flex-wrap items-center justify-center gap-2 rounded-2xl border border-slate-200/90 bg-white/95 px-3 py-2 shadow-lg backdrop-blur-md dark:border-slate-600 dark:bg-slate-900/95">
        {s.kind === 'rect' || s.kind === 'ellipse' || s.kind === 'triangle' || s.kind === 'kite' ? (
          <>
            <label className="flex cursor-pointer items-center gap-1.5">
              ...
              />
              <span className="text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400">Fill</span>
            </label>
            <FillColorSwatch
              label="Color"
              value={s.fill ?? '#2962ff'}
              ...
              onPickColor={(hex) =>
                updateShape(s.id, {
                  ...s,
                  fill: hex,
                  fillOpacity: (s.fillOpacity ?? 1) <= 0 ? 1 : (s.fillOpacity ?? 1),
                } as DrawShape)
              }
            />
            <OpacitySlider
              ...
              onChange={(fillOpacity) => updateShape(s.id, { ...s, fillOpacity } as DrawShape)}
            />
            <ColorSwatch
              label="Border"
              value={s.stroke}
              onChange={(stroke) => updateShape(s.id, { ...s, stroke } as DrawShape)}
            />
            <StrokeWidthSlider
              value={s.strokeWidth}
              onChange={(strokeWidth) => updateShape(s.id, { ...s, strokeWidth } as DrawShape)}
            />
            <StrokeDashSelect
              value={s.strokeDash}
              onChange={(strokeDash) => updateShape(s.id, { ...s, strokeDash } as DrawShape)}
            />
          </>
        ) : null}
        ...
        {s.kind === 'text' ? (
          <ColorSwatch
            label="Text"
            value={s.fill}
            onChange={(fill) => updateShape(s.id, { ...s, fill } as DrawShape)}
          />
        ) : null}
        ...
        {s.kind === 'sticky' ? (
          <>
            ...
            <ColorSwatch
              label="Text"
              value={s.textColor}
              onChange={(textColor) => updateShape(s.id, { ...s, textColor } as DrawShape)}
            />
          </>
        ) : null}
      </div>
