...
export function CommittedShapeNode({
  ...
  const { interactive, onSelect, onChange, innerRef, isEditing = false, onBeginEdit } = interaction;
      ...
      return (
        <Line
          ...
          onDragEnd={(e) => {
            ...
            onChange({
              ...s,
              points: s.points.map((v: number, i: number) => (i % 2 === 0 ? v + dx : v + dy)),
            });
          }}
...
export function DraftShapeNode({ draft: d, stroke, stickyFill }: { draft: Draft; stroke: string; stickyFill: string }) {
  const dash = [6, 6];
  if (d.kind === 'pen') {
  ...
  if (d.kind === 'sticky') {
    ...
    return <Rect x={x} y={y} width={w} height={h} fill={stickyFill} opacity={0.9} cornerRadius={8} listening={false} />;
  }
  if (d.kind === 'rect') {
    const x = Math.min(d.x1, d.x2);
    const y = Math.min(d.y1, d.y2);
