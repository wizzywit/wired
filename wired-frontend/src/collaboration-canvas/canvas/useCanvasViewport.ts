...
export function useCanvasViewport(
  ...
  const clientToWorld = useCallback(
    (clientX: number, clientY: number) => {
      return clientToWorldFromStage(clientX, clientY, stageRef.current?.container() ?? null, size.w, size.h, viewport);
    },
  ...
  const handleWheel = useCallback(
    (e: KonvaEventObject<WheelEvent>) => {
      ...
      setViewport((prev) => zoomWheelAtPointer(prev, pointer, size.w, size.h, scaleBy));
    },
    [size.w, size.h]
  );

  const handlePanMouseMove = useCallback((e: KonvaEventObject<MouseEvent>) => {
