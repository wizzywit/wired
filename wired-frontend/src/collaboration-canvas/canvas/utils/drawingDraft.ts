...
export function createDraftFromTool(tool: CanvasTool, w: { x: number; y: number }): Draft | null {
  if (tool === 'pen') {
...
export function mergeDraftWithPoint(draft: Draft, p: { x: number; y: number }, penMinDist: number): Draft {
  if (draft.kind === 'pen') {
...
export function commitDraft(d: Draft, deps: CommitDeps): DrawShape | null {
  const { stroke, newId, minShapePx, stickyFill, stickyTextColor, shapeFill, shapeFillOpacity } = deps;
