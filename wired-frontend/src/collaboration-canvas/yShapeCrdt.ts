...
function writeCommon(inner: Y.Map<unknown>, id: string, kind: DrawShape['kind']) {
  yStr(inner, 'id', id);
  ...
  switch (kind) {
    ...
    case 'text': {
      ...
      const fs =
        fontStyle === 'normal' || fontStyle === 'bold' || fontStyle === 'italic' || fontStyle === 'bold italic'
          ? fontStyle
          : undefined;
      const al = align === 'left' || align === 'center' || align === 'right' ? align : undefined;
      const t: DrawShape = {
        id,
        kind: 'text',
        x,
    ...
    case 'sticky': {
      ...
      return {
        ...
        fontFamily: readStr(inner, 'fontFamily'),
        fontStyle: readStr(inner, 'fontStyle') as 'normal' | 'bold' | 'italic' | 'bold italic' | undefined,
