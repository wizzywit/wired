...
export function CanvasTextEditOverlay({
  ...
  const shape: EditableShape | undefined =
    editingId == null
      ? undefined
      : shapes.find((s): s is EditableShape => s.id === editingId && isEditable(s))
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (shape.kind === 'sticky') {
        ...
        const ta = measureStickyTextArea(
          node,
          stage,
          w,
          h,
  ...
  const bumpFontSize = useCallback(
    (delta: number) => {
      if (!shape) return
      const base =
        shape.kind === 'text' ? shape.fontSize : (shape.fontSize ?? 13)
      applyPatch({
  ...
  const editBg =
    shape.kind === 'sticky'
      ? hexToRgba(shape.fill, shape.fillOpacity ?? 1)
      : 'transparent'
  ...
  return (
    <>
      ...
      >
        <div className="pointer-events-auto flex max-w-[min(100vw-2rem,42rem)] flex-wrap items-center justify-center gap-1 rounded-2xl border border-slate-200/90 bg-white/95 px-2 py-1.5 shadow-lg backdrop-blur-md dark:border-slate-600 dark:bg-slate-900/95">
          ...
          <select
            ...
            value={
              TEXT_FONT_OPTIONS.find((o) => o.value === fontFamily)?.id ?? 'inter'
            }
            onChange={(e) => {
      ...
      <textarea
        ...
        value={shape.text}
        onChange={(e) =>
          applyPatch({ text: e.target.value } as Partial<EditableShape>)
        }
      />
    </>
  )
