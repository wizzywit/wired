...
export async function getDocumentShareState(documentId: string) {
  return apiRequest<{ share: DocumentShareState }>(`/documents/${encodeURIComponent(documentId)}/share`, {
    method: 'GET',
  });
}
...
export async function removeDocumentCollaborator(documentId: string, userId: string) {
  return apiRequest(`/documents/${encodeURIComponent(documentId)}/share/users/${encodeURIComponent(userId)}`, {
    method: 'DELETE',
  });
}

export async function updateDocumentLinkAccess(documentId: string, access: LinkAccess) {
  return apiRequest<{ ok: true }>(`/documents/${encodeURIComponent(documentId)}/share/link-access`, {
    method: 'PATCH',
...
export async function revokeDocumentInvite(documentId: string, email: string) {
  return apiRequest(`/documents/${encodeURIComponent(documentId)}/share/invites`, {
    method: 'DELETE',
